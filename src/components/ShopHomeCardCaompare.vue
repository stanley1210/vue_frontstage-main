...
const emit = defineEmits(["likeCreate", "updateCarId"]);
...
onMounted(async () => {
  ...
  axiosapi
    .get(`/image/isMainPic/${props.shopHomeCard.id}`)
    .then(function (response) {
      if (response && response.data) {
        console.log("response", response);
        isMainPic.value = response.data.isMainPic;
        console.log("isMainPic", isMainPic.value);
      } else {
...
function pushCarId() {
  emit("updateCarId", { carId: props.shopHomeCard.id });
}
...
function navigateToCar() {
  if (customerId.value) {
    ...
    axiosapi
      .get(`/carinfo/find/${props.shopHomeCard.carinfoId}`)
      .then(function (responce) {
        ...
        axiosapi
          .get(`/customerRecord/modify/${customerId.value}`, {
            params: {
              brand: carInfo.brand,
              cc: carInfo.cc,
              door: carInfo.door,
          ...
          .then(function (responce) {
            ...
            if (!responce.data.data) {
              axiosapi
                .post(`/customerRecord`, { customerId: customerId.value })
                .then(function (responce) {
                  console.log("建立新推薦紀錄");
                })
                .catch(function (error) {
  ...
  router.push({
    name: "pages-shop-car-link",
    query: { carId: props.shopHomeCard.id },
  });
}
