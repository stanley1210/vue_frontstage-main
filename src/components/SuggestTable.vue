<template>
  <div style="margin-bottom: 5%">
    <div class="slogan" style="color: #9a7352">
      <h1>
        Check these out,<br />
        You might also Find your next Favorite ride here!
      </h1>
    </div>
    <div
    ...
    >
      <ShopHomeCard
        v-for="shopSuggestCard in shopSuggestCards"
        :key="shopSuggestCard.id"
        :shopHomeCard="shopSuggestCard"
      ></ShopHomeCard>
    </div>
  </div>
</template>
    ...
    .then(function (responce) {
      ...
      if (customerRecord.value.no01Care != null) {
        ...
        switch (Math.floor(customerRecord.value.no01Care / 10)) {
          case 0:
            brandId.value = customerRecord.value.no01Care % 10;
            break;
          case 1:
            displacementId.value = customerRecord.value.no01Care % 10;
            break;
          case 2:
            doorId.value = customerRecord.value.no01Care % 10;
            break;
          case 3:
            passengerId.value = customerRecord.value.no01Care % 10;
            break;
          case 4:
            gasolineId.value = customerRecord.value.no01Care % 10;
            break;
          case 5:
            rearWheelId.value = customerRecord.value.no01Care % 10;
            break;
          case 6:
            suspensionId.value = customerRecord.value.no01Care % 10;
            break;
          case 7:
            transmissionId.value = customerRecord.value.no01Care % 10;
            break;
          default:
            break;
        }
        ...
        if (temp.length <= 3) {
          axiosapi
            ...
            .then(function (response) {
              ...
              console.log(
                "shopSuggestCards length 2",
                shopSuggestCards.value.length
              );
            });
        }
      } else {
